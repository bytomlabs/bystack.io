    ...
    return (
      <Layout homeDisable ftStyle={{marginTop: 0}}>
        ...
        <Cont>
          <Head>
            ...
            <Toolbar>
              <div>
                ...
                <Link to={'/record'}><Msg id="vote_mobile_record" /></Link>
                <Link to={'/rules'}><Msg id="vote_mobile_rule" /></Link>
              </div>
              <Link to={'/vote_reward'}>投票收益</Link>
              {/* <Input
          ...
          {
            list.map((item, index) => (
              <Link key={index} to={`/node_detail?id=${item.uuid}`}>
                <Node>
